...
<div class="gycpfl-top"><span>${menu.menuName}</span></div>
<div class="gycpfl-bottom">
 <ul> 
 <%
  List<web_commoninfo> list=(List<web_commoninfo>)request.getAttribute("list_commoninfo");
  if(list.size()>0)
  {
  Iterator<web_commoninfo> it=list.iterator();  
  web_commoninfo news=null;  
  int seq=0;
  while(it.hasNext())
  {
  seq++;
    news=(web_commoninfo)it.next();
    %>
    <li> <img src="/resources/template/company/images/jqlb_12.jpg" width="14" height="11" />&nbsp;<a href="?id=<%=news.getCommoninfoid()%>"><%=news.getCmmName()%></a></li>
    <%
    }
 ...
 <ul>
 <li><a> ${member.memberRealname}</a></li>
