...
<div id="right">
  ...
  <span>${menu.menuName}</span>&gt;&gt;<span><c:if test="${! empty supplyclass}">${supplyclass.scName}</c:if></span>  </div>
 <div class="body_nr">
<ul>
<c:if test="${! empty list_supplyinfo}">
<%
...
Iterator<web_supplyinfo> it=list.iterator();  
  ...
  seq++;
    ...
    %>
    <li> 
<a id="link" href="/${username}/purvey/${menu.menuid}/${supplyclass.scid}/<%=product.getSupplyinfoid()%>"> <span id="gq_left">【<%=strArray[product.getSupplyinfoType()-1]%>】</span> <%=product.getSupplyinfoTitle()%> </a>    </li>
<li>
